...
class PaymentWebhook(models.Model):
    """
    Record of webhooks received from payment providers
    """
    PROVIDER_CHOICES = [
        ('stripe', 'Stripe'),
        ('paypal', 'PayPal'),
    ...
    provider = models.CharField('Provider', max_length=20, choices=PROVIDER_CHOICES)
    event_type = models.CharField('Event Type', max_length=100)
    event_id = models.CharField('Event ID', max_length=255, unique=True)
    
    # Webhook data
    raw_payload = models.JSONField('Raw Payload')
    headers = models.JSONField('Headers', default=dict)
    signature = models.TextField('Signature', blank=True)
    
    # Processing status
    status = models.CharField('Status', max_length=20, choices=STATUS_CHOICES, default='received')
    processed_at = models.DateTimeField('Processed At', null=True, blank=True)
    error_message = models.TextField('Error Message', blank=True)
    
    # Relationship with payments (optional)
    payment = models.ForeignKey(
        Payment,
        on_delete=models.SET_NULL,
        null=True,
...
# Signals to automate processes
from django.db.models.signals import post_save
from django.dispatch import receiver

@receiver(post_save, sender=Payment)
def create_payment_transaction(sender, instance, created, **kwargs):
    """Create automatic transaction when a payment is created"""
    if created:
        PaymentTransaction.objects.create(
            payment=instance,
            ...
            success=False,  # Updated when payment is successful
